...
class Transaction(core.models.sqlalchemy.Base):
    ...
    sell_order_id: Mapped[Uuid] = mapped_column(Uuid, ForeignKey("orders.id"))
    buy_order_id: Mapped[Uuid] = mapped_column(Uuid, ForeignKey("orders.id"))
    qty: Mapped[int]
    price: Mapped[int]
    ...
    __table_args__ = (
        CheckConstraint("qty > 0", name="check_transaction_qty_positive"),
